...

  {
    ...
    translations: {
      fr: {
        ...
        description:
          "K-Study Hub est une application web moderne d’apprentissage du coréen combinant un dictionnaire coréen-français bidirectionnel et un système de quiz interactif. Développée avec Next.js et React, elle propose une recherche performante alimentée par l’API officielle KRDict, un quiz aléatoire de plus de 500 mots de vocabulaire, un système de favoris persistant ainsi qu’un historique de recherche. Ce projet a été réalisé en collaboration avec une intelligence artificielle, dans une démarche d’apprentissage visant à maîtriser cet outil comme assistant de développement. Cet assistant a servi de partenaire de travail pour explorer, comprendre et implémenter ces technologies — le développement restant entièrement guidé par mes décisions et ma compréhension du code. Le projet démontre ma capacité à concevoir une application moderne avec une UX soignée, des thèmes clair/sombre et une architecture prête pour le déploiement.",
        context:
          "Passionné par la langue coréenne, j’ai développé K-Study Hub afin de créer un outil d’apprentissage complet et accessible. L’application utilise l’API officielle du dictionnaire coréen (KRDict - 한국어기초사전) pour fournir des définitions précises accompagnées de traductions françaises. Le quiz repose sur un vocabulaire statique de plus de 500 mots couvrant plus de 25 catégories (famille, nourriture, verbes, couleurs, expressions, etc.), garantissant une expérience fiable sans dépendre des quotas API. L’interface moderne avec mode clair/sombre s’adapte automatiquement aux préférences du système.",
        Objectives:
          "Développer une application Next.js complète avec App Router et Server Components. Intégrer l’API KRDict pour la recherche bidirectionnelle coréen-français et français-coréen. Concevoir un système de quiz interactif avec plus de 500 mots de vocabulaire organisés par catégories. Implémenter la persistance locale via localStorage (favoris, historique, scores, préférences de thème). Créer une interface responsive et accessible avec prise en charge des thèmes clair/sombre. Préparer l’application pour un déploiement sur Vercel avec gestion des variables d’environnement.",
        SkillsDeveloped:
          "Next.js & React : App Router, Server Components, route handlers API, hooks personnalisés (useLocalStorageState), architecture modulaire. TypeScript : typage strict des données API, interfaces pour les entrées du dictionnaire et les questions de quiz. Intégration API : consommation de l’API KRDict en XML, parsing avec fast-xml-parser, gestion des erreurs et mécanismes de fallback. UI/UX moderne : Tailwind CSS v4, thèmes dynamiques, animations CSS, design responsive mobile-first. Persistance côté client : gestion de l’état avec localStorage, hydratation SSR correcte, synchronisation des préférences utilisateur.",
        Results:
          "Une application dictionnaire pleinement fonctionnelle avec recherche bidirectionnelle coréen-français. Un quiz interactif de plus de 500 mots avec suivi du score et possibilité de réinitialisation. Un système de favoris permettant de sauvegarder les mots importants. Un historique de recherche avec horodatage et indication du sens de recherche. Une interface moderne et responsive avec mode clair/sombre automatique. Une application prête pour la production, déployable sur Vercel ou Netlify.",
      },
      ...
      en: {
        ...
        description:
          "K-Study Hub is a modern Korean language learning web application that combines a bidirectional Korean-French dictionary with an interactive quiz system. Built with Next.js and React, it features powerful search powered by the official KRDict API, a random quiz with over 500 vocabulary words, a persistent favorites system, and a search history. This project was developed in collaboration with artificial intelligence as a learning approach to mastering the use of this tool as a development assistant. The assistant acted as a working partner to explore, understand, and implement these technologies — while all development decisions and code comprehension remained fully my own. The project demonstrates my ability to build a modern application with polished UX, light/dark themes, and a deployment-ready architecture.",
        context:
          "Passionate about the Korean language, I developed K-Study Hub to create a comprehensive and accessible learning tool. The application uses the official Korean Dictionary API (KRDict - 한국어기초사전) to deliver accurate definitions with French translations. The quiz relies on a static vocabulary of over 500 words across 25+ categories (family, food, verbs, colors, expressions, etc.), ensuring a reliable experience without dependency on API quotas. The modern interface with light/dark mode automatically adapts to system preferences.",
        Objectives:
          "Develop a complete Next.js application using App Router and Server Components. Integrate the KRDict API for bidirectional Korean-French and French-Korean search. Build an interactive quiz system with over 500 categorized vocabulary words. Implement local persistence using localStorage (favorites, history, scores, theme preferences). Design a responsive and accessible interface with light/dark theme support. Prepare the application for deployment on Vercel with proper environment variable management.",
        SkillsDeveloped:
          "Next.js & React: App Router, Server Components, API route handlers, custom hooks (useLocalStorageState), modular architecture. TypeScript: strict typing of API data, interfaces for dictionary entries and quiz questions. API Integration: consuming the KRDict XML API, parsing with fast-xml-parser, error handling and fallback mechanisms. Modern UI/UX: Tailwind CSS v4, dynamic themes, CSS animations, mobile-first responsive design. Client-side persistence: state management with localStorage, proper SSR hydration, synchronization of user preferences.",
        Results:
          "A fully functional dictionary application with bidirectional Korean-French search. An interactive quiz featuring over 500 words with score tracking and reset functionality. A favorites system allowing users to save important vocabulary. A search history with timestamps and direction indicators. A modern, responsive interface with automatic light/dark mode. A production-ready application deployable to Vercel or Netlify.",
      },
      ...
      ko: {
        ...
        description:
          "K-Study Hub는 양방향 한국어-프랑스어 사전과 인터랙티브 퀴즈 시스템을 결합한 현대적인 한국어 학습 웹 애플리케이션입니다. Next.js와 React로 개발되었으며, 국립국어원 공식 KRDict API를 활용한 강력한 검색 기능, 500개 이상의 어휘를 포함한 랜덤 퀴즈, 즐겨찾기 시스템, 검색 기록 기능을 제공합니다. 이 프로젝트는 개발 어시스턴트로서 인공지능을 활용하는 방법을 학습하기 위한 과정의 일환으로 협업하여 제작되었습니다. 해당 어시스턴트는 기술을 탐구하고 이해하며 구현하는 작업 파트너 역할을 했으며, 모든 개발 과정은 전적으로 나의 판단과 코드 이해를 기반으로 진행되었습니다. 이 프로젝트는 세련된 UX, 라이트/다크 테마, 그리고 배포 준비가 완료된 아키텍처를 갖춘 현대적인 애플리케이션을 구현할 수 있는 나의 역량을 보여줍니다.",
        context:
          "한국어에 대한 열정을 바탕으로, 종합적이고 접근성 높은 학습 도구를 만들기 위해 K-Study Hub를 개발했습니다. 이 애플리케이션은 공식 한국어 사전 API(KRDict - 한국어기초사전)를 사용하여 프랑스어 번역이 포함된 정확한 정의를 제공합니다. 퀴즈는 가족, 음식, 동사, 색상, 표현 등 25개 이상의 카테고리를 아우르는 500개 이상의 정적 어휘를 기반으로 구성되어 있으며, API 할당량에 의존하지 않고 안정적인 학습 경험을 제공합니다. 라이트/다크 모드를 지원하는 현대적인 인터페이스는 시스템 설정에 자동으로 적응합니다.",
        Objectives:
          "App Router와 Server Components를 활용한 완전한 Next.js 애플리케이션을 개발한다. 양방향 한국어-프랑스어 및 프랑스어-한국어 검색을 위해 KRDict API를 통합한다. 카테고리별로 구성된 500개 이상의 어휘를 포함하는 인터랙티브 퀴즈 시스템을 구현한다. 즐겨찾기, 기록, 점수, 테마 설정을 위한 localStorage 기반 로컬 영속성을 구현한다. 라이트/다크 테마를 지원하는 반응형이며 접근성 높은 인터페이스를 설계한다. 환경 변수 관리를 포함하여 Vercel 배포를 준비한다.",
        SkillsDeveloped:
          "Next.js & React: App Router, Server Components, API 라우트 핸들러, 커스텀 훅(useLocalStorageState), 모듈형 아키텍처. TypeScript: API 데이터에 대한 엄격한 타입 정의, 사전 항목 및 퀴즈 질문 인터페이스 설계. API 통합: KRDict XML API 사용, fast-xml-parser를 통한 파싱, 오류 처리 및 폴백 로직 구현. 현대적인 UI/UX: Tailwind CSS v4, 동적 테마, CSS 애니메이션, 모바일 우선 반응형 디자인. 클라이언트 사이드 영속성: localStorage 기반 상태 관리, 올바른 SSR 하이드레이션, 사용자 설정 동기화.",
        Results:
          "양방향 한국어-프랑스어 검색이 가능한 완전한 사전 애플리케이션. 점수 추적 및 초기화 기능을 갖춘 500개 이상의 어휘 기반 인터랙티브 퀴즈. 중요한 단어를 저장할 수 있는 즐겨찾기 시스템. 타임스탬프와 검색 방향 표시가 포함된 검색 기록. 자동 라이트/다크 모드를 지원하는 현대적인 반응형 인터페이스. Vercel 또는 Netlify에 배포 가능한 프로덕션 준비 애플리케이션.",
      },
    },
